...
text = open(path + "..\WorldData\Faces.txt", "w") # file path
...
for index in range(0, 2): # column index
    for row in range(0, 16): # row index
        for chunk in chunkArray[index]: # chunk
            faces = np.array(chunk.data.polygons).reshape(16,16) # 2D vertex array
            for faceColumn in range(0, 16): # column index
                coords = chunk.matrix_world @ faces[row][faceColumn].center # vertex location
                for c in coords[:-1]: # x, y, z coords
                    text.write(str(c) + ",")
                text.write(str(coords[-1]) + "|")
        text.write(n)
